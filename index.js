let indiceCount = 0; // Variable pour enregistrer le nombre d'indices demandés
    ...
    let currentInputIndex = 0; // Indice du mot actuellement affiché dans l'onglet Input
  
    const inputText = document.getElementById('inputText');
    const prevInputBtn = document.getElementById('prevInputBtn');
    const nextInputBtn = document.getElementById('nextInputBtn');
  
    function displayInputWord(index) {
      inputText.innerText = inputWords[index];
    }
  
    displayInputWord(currentInputIndex);
  
    prevInputBtn.addEventListener('click', function() {
    ...
    nextInputBtn.addEventListener('click', function() {
      if (currentInputIndex < inputWords.length - 1) {
        currentInputIndex++;
        displayInputWord(currentInputIndex);
      }
    });
